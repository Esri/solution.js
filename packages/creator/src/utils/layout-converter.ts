 ...
 */
export function convertLayoutToTemplate(layout: any): any {
  if (!layout) {
    return layout;
  }

  // walk the sections, rows, cards... then call to fn's to convert specific cards...
  const converted = layout.sections.reduce(
    (acc: any, section: any) => {
 ...
 */
export function convertSection(section: any): any {
  ...
  const rowResult = section.rows.reduce(
    (acc: any, row: any) => {
...
export function extractAssets(obj: any): any {
  const assets = [];
  if (obj.fileSrc) {
 ...
 */
export function convertRow(row: any): any {
  // if the section has a background image, and it has a url, we should
  // add that to the asset hash so it can be downloaded and added to the template item
  // and also cook some unique asset name so we can inject a placeholder
  return row.cards.reduce(
    (acc: any, card: any) => {
      // convert the card...
 ...
 */
export function convertCard(card: any): any {
 ...
 */
export function convertImageCard(card: any): any {
  const result = {
    card,
    assets: [] as any[]
  };
 ...
 * @returns Hash including the converted card, and any assets
 */
export function convertJumbotronCard(card: any): any {
  const result = {
    card,
    assets: [] as any[]
  };
 ...
 * @returns Hash including the converted card, and any assets
 */
export function convertItemGalleryCard(card: any): any {
 ...
 * @returns Hash including the converted card, and any assets
 */
export function convertFollowCard(card: any): any {
 ...
 * @returns Hash including the converted card, and any assets
 */
export function convertEventListCard(card: any): any {
