 ...
 */
export function getCascadeDependencies(model: any): string[] {
  ...
  return sections.reduce((a: any, s: any) => {
    return a.concat(
      getDeepValues(s, "webmap").map((e: any) => {
        return e.id;
      })
    );
  }, []);
 ...
 */
export function getMapSeriesDependencies(model: any): string[] {
...
export function getMapJournalDependencies(model: any): string[] {
