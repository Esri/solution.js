...
export function convertItemToTemplate(
  ...
  return new Promise((resolve, reject) => {
    ...
    items.getItem(itemId, requestOptions).then(
      itemResponse => {
        if (!moduleMap[itemResponse.type.toLowerCase()]) {
          console.warn(
            "Unimplemented item type " + itemResponse.type + " for " + itemId
          );
        }
        ...
        Promise.all([
          ...
          itemTemplate.resources =
            resourceResponse && resourceResponse.total > 0
              ? resourceResponse.resources.map((resourceInfo: any) => {
                  return (
                    requestOptions.portal +
                    "/content/items/" +
                    itemId +
      ...
      () => {
        ...
        groups.getGroup(itemId, requestOptions).then(
          itemResponse => {
            itemTemplate = {
              itemId: itemResponse.id,
              type: "Group",
 ...
 */
export function initItemTemplateFromJSON(itemTemplate: ITemplate): ITemplate {
  itemTemplate.fcns =
    moduleMap[itemTemplate.type.toLowerCase()] || GenericModule;
  return itemTemplate;
}
 ...
 * @protected
 */
export function flatten(nestedArray = [] as string[]): string[] {
 ...
 * @protected
 */
export function removeDuplicates(arrayWithDups = [] as string[]): string[] {
  const uniqueStrings: {
    [value: string]: boolean;
  } = {};
  arrayWithDups.forEach(
    (arrayElem: string) => (uniqueStrings[arrayElem] = true)
  );
 ...
 * @protected
 */
export function removeUndesirableItemProperties(item: any): any {
  if (item) {
