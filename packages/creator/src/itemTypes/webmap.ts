...
export function convertItemToTemplate(
  ...
  return new Promise(resolve => {
    ...
    itemTemplate.item.url =
      mCommon.PLACEHOLDER_SERVER_NAME +
      WEBMAP_APP_URL_PART +
...
export function createItemFromTemplate(
  ...
  return new Promise((resolve, reject) => {
    ...
    items.createItemInFolder(options).then(
      createResponse => {
 ...
 * @protected
 */
export function extractDependencies(itemTemplate: ITemplate): string[] {
 ...
 * @protected
 */
export function getWebmapLayerIds(layerList = [] as any[]): string[] {
...
export function templatizeWebmapLayerIdsAndUrls(layerList = [] as any[]): void {
  layerList
    ...
    .forEach((layer: any) => {
      ...
      layer.url =
        mCommon.templatize(mCommon.deTemplatize(layer.itemId), "url") + layerId;
    });
}
