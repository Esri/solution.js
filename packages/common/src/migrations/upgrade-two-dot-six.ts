...
import { getPortal } from "../restHelpersGet";
...
export function _upgradeTwoDotSix(
  ...
  if (getProp(model, "item.properties.schemaVersion") >= 2.6) {
    ...
    // fetch org details
    return getPortal("", authentication)
      .then(portal => {
        const {
        ...
        return Promise.all([
          // get the orgs default base map group
          searchGroups(basemapGalleryGroupQuery, authentication),
          Promise.resolve(basemapTitle)
        ]);
      })
      .then(([baseMapGroupResults, basemapTitle]) => {
        const {
          results: [baseMapGroup]
        } = baseMapGroupResults;
        // search for the orgs default base map within the group
            ...
            formTemplate.properties.form.questions.forEach((question: any) => {
              if (question.maps) {
                question.maps.forEach((webMap: any) => {
                  ...
                  // override each web map to the default org web map
                  webMap.itemId = baseMap.id;
                });
              }
            });
          }
