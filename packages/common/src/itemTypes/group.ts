...
export function convertItemToTemplate(
  ...
  return new Promise((resolve, reject) => {
    ...
    getGroupContents(itemTemplate, requestOptions).then(
      dependencies => {
    ...
    groups.createGroup(options).then(
      createResponse => {
        if (createResponse.success) {
          ...
          addGroupMembers(itemTemplate, requestOptions, progressCallback).then(
            () => {
...
export function addGroupMembers(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    if (itemTemplate.dependencies.length > 0) {
      ...
      Promise.all(awaitGroupAdds).then(
        () => resolve(),
        e => reject(mCommon.fail(e))
      );
    } else {
...
export function getGroupContents(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Fetch group items
    getGroupContentsTranche(itemTemplate.itemId, pagingRequest).then(
      contents => {
        // Update the estimated cost factor to deploy this item
      ...
      contents => {
        if (contents.num > 0) {
          ...
          if (contents.nextStart > 0) {
            ...
            getGroupContentsTranche(id, pagingRequest).then(
