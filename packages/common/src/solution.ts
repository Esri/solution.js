  ...
  return new Promise<mInterfaces.ISolutionItem>((resolve, reject) => {
    ...
    createSolutionAgoItem(
      ...
      solutionItem => {
        ...
        createItemTemplates(ids, sourceRequestOptions).then(
          templates => {
            ...
            createSolutionStorageAgoItem(
              ...
              solutionStorageItem => {
                solutionItem.data.metadata.resourceStorageItemId =
                  solutionStorageItem.item.id;

                // Save the source item thumbnails and resources
                const saveResourcesDef = saveResourcesInSolutionItem(
                ...
                const saveTemplatesDef = updateSolutionAgoItem(
                  solutionItem,
                  destinationRequestOptions
                );

                Promise.all([saveResourcesDef, saveTemplatesDef]).then(
                  responses => {
...
export function deploySolutionItem(
  ...
  return new Promise<mInterfaces.ITemplate[]>((resolve, reject) => {
    const templates: mInterfaces.ITemplate[] = mObjHelpers.getProp(
      solutionItem,
      "data.templates"
    );
    ...
    if (settings.folderId) {
      ...
      const folderName =
        settings.solutionName + " (" + mCommon.getUTCTimestamp() + ")";
      const options = {
        title: folderName,
        authentication: requestOptions.authentication
      };
      items.createFolder(options).then(
...
export function getEstimatedDeploymentCost(
  ...
  const reducer = (
    accumulator: number,
    currentTemplate: mInterfaces.ITemplate
  ) =>
    accumulator +
    (currentTemplate.estimatedDeploymentCostFactor
      ? currentTemplate.estimatedDeploymentCostFactor
      : 3);
 ...
 * @protected
 */
export const OPS_DASHBOARD_APP_URL_PART: string =
      ...
      content => {
        ...
        content.blob().then(
          (blob: any) => {
            ...
            items.addItemResource(addRsrcOptions).then(
              () => resolve(resourceTag),
...
export function copyThumbnailResource(
  ...
  let filename = url.substring(
    url.indexOf("/info/thumbnail/") + "/info/thumbnail/".length
  );
  if (itemType === "Group") {
    folder = itemId + "_info";
 ...
 * @protected
 */
function createPlaceholderTemplate(id: string): mInterfaces.ITemplate {
  return {
    itemId: id,
    type: "",
    key: "",
...
export function createDeployedSolutionAgoItem(
  ...
  return new Promise((resolve, reject) => {
    ...
    const thumbnailUrl: string =
      "https://www.arcgis.com/sharing/content/items/" +
      templateItem.id +
      "/info/" +
    ...
    mCommon
      ...
      .then(
        createResponse => {
          const orgUrl =
            (settings.organization && settings.organization.orgUrl) ||
...
export function createItemFromTemplateWhenReady(
  ...
  const itemDef = new Promise<mInterfaces.ITemplate>((resolve, reject) => {
    ...
    (template.dependencies || []).forEach(dependencyId =>
      awaitDependencies.push(settings[dependencyId].def)
    );
    Promise.all(awaitDependencies).then(
      () => {
        // Prepare template
        let itemTemplate = mClassifier.initItemTemplateFromJSON(
          findTemplateInList(itemTemplates, itemId)
        );
        ...
        itemTemplate.fcns
          ...
          .then(itemClone => resolve(itemClone), e => reject(mCommon.fail(e)));
      },
      e => reject(mCommon.fail(e))
    );
  });
  ...
  return new Promise((resolve, reject) => {
    if (typeof ids === "string") {
      ...
      if (findTemplateInList(existingTemplates, rootId)) {
        ...
        // Init item type
        const getItemPromise = mClassifier.convertItemToTemplate(
          rootId,
          sourceRequestOptions
        );
      ...
      ids.forEach(id => {
        getHierarchyPromise.push(
          createItemTemplates(id, sourceRequestOptions, existingTemplates)
        );
      });
      Promise.all(getHierarchyPromise).then(
        () => {
          resolve(existingTemplates);
        },
        e => reject(mCommon.fail(e))
    ...
    mCommon
      ...
      .then(
        createResponse => {
          ...
          solutionItem.item.url =
            orgUrl + "/home/item.html?id=" + createResponse.id;
          resolve(solutionItem);
        },
        e => reject(mCommon.fail(e))
      );
...
export function getFolderAndFilenameForResource(
  ...
  let folder = itemId;
  let filename = url.substring(
    url.indexOf("/resources/") + "/resources/".length
  );
...
export function publishSolutionTemplate(
  ...
  return mCommon.createItemWithData(
    item,
    data,
    requestOptions,
    folderId,
...
export function saveResourcesInSolutionItem(
  ...
  return new Promise<string[]>((resolve, reject) => {
    ...
    if (templates) {
      templates.forEach(itemTemplate => {
        ...
        if (itemTemplate.item.thumbnail) {
          copiesDefList.push(
            copyThumbnailResource(
              itemTemplate.itemId,
              itemTemplate.item.thumbnail as string,
              itemTemplate.type,
              storageItemId,
              sourceRequestOptions,
              destinationRequestOptions
        ...
        if (itemTemplate.resources) {
          itemTemplate.resources.forEach(resourceUrl => {
            copiesDefList.push(
              copyRegularResource(
                itemTemplate.itemId,
    ...
    Promise.all(copiesDefList).then(resolve, e => reject(mCommon.fail(e)));
  });
}
...
function updateSolutionAgoItem(
  ...
  return new Promise<mInterfaces.ISolutionItem>((resolve, reject) => {
    ...
    mCommon
      .updateItemData(solutionItem.item.id, solutionItem.data, requestOptions)
      .then(() => resolve(solutionItem), e => reject(mCommon.fail(e)));
  });
}
