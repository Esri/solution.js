...
describe("Module `restHelpersGet`: common REST fetch functions shared across packages", () => {
  ...
  describe("getOrganizationSettings", () => {
    it("can handle an exception on get organization", async () => {
      ...
      fetchMock.get(new RegExp(`${utils.PORTAL_SUBSET.restUrl}/community/self.*`), mockItems.get500Failure());
    ...
    it("can get the organization's settings", async () => {
      ...
      fetchMock.get(new RegExp(`${utils.PORTAL_SUBSET.restUrl}/community/self.*`), response);
    ...
    it("returns empty object when orgSettings is missing", async () => {
      ...
      fetchMock.get(new RegExp(`${utils.PORTAL_SUBSET.restUrl}/community/self.*`), responseWithoutOrgSettings);
