...
describe("Module `trackingHelpers`: common functions", () => {
  ...
  describe("setLocationTrackingEnabled", () => {
    it("locationTracking false and role not admin", () => {
      ...
      setLocationTrackingEnabled(portalResponse, userResponse, templateDictionary);
  ...
  describe("_validateTrackingTemplates", () => {
    it("handles no typeKeywords", () => {
      const templates: IItemTemplate[] = [mockTemplates.getItemTemplate("Feature Service")];
    ...
    it("throws error if we have tracking templates and tracking not enabled", () => {
      ...
      expect(() => _validateTrackingTemplates(templates, templateDictionary)).toThrow(
  ...
  describe("templatizeTracker", () => {
    it("should templatize group prop for location tracking views", () => {
      ...
      expect(_itemTemplate.item.properties.trackViewGroup).toEqual("{{aaad83aae2bc4cec884c165d9d0c9988.itemId}}");
      expect(_itemTemplate.dependencies).toEqual(["aaad83aae2bc4cec884c165d9d0c9988"]);
      expect(_itemTemplate.groups).toEqual(["aaad83aae2bc4cec884c165d9d0c9988"]);
      expect(_itemTemplate.properties.layers[0].adminLayerInfo.viewLayerDefinition.sourceServiceItemId).toEqual(
        "{{bbbd83aae2bc4cec884c165d9d0c9988.itemId}}",
      );
      expect(_itemTemplate.item.name).toEqual("{{aaad83aae2bc4cec884c165d9d0c9988.itemId}}_Track_View");
    });
