...
describe("Module `templatization`: common functions involving the adlib library", () => {
  ...
  describe("createInitializedGroupTemplate", () => {
    ...
    it("inserts typeKeywords", () => {
      const template: interfaces.IItemTemplate = templatization.createInitializedGroupTemplate({
        id: "fred",
        typeKeywords: ["a", "b"],
      });
      template.key = "";
  ...
  describe("extractSolutionVersion", () => {
    it("defaults to a version 0 solution", () => {
      const itemData: interfaces.ISolutionItemData = mockItems.getAGOLItemData("Solution");
  ...
  describe("findTemplateIndexInList", () => {
    ...
    it("should handle list without the sought item", () => {
      const solnTemplates = createItemTemplateList(["abc", "def", "ghi", "jkl"]);
      const id: string = "xyz";
      const expected: number = -1;
  ...
  describe("findTemplateInList", () => {
    ...
    it("should handle list with the sought item", () => {
      const solnTemplates = createItemTemplateList(["abc", "def", "ghi", "jkl"]);
      const id: string = "def";
      const expected: interfaces.IItemTemplate = createItemTemplateList(["def"])[0];
  ...
  describe("getIdsInTemplatesList", () => {
    it("gets the ids out of a list of templates", () => {
      ...
      expect(actual).toEqual(["map1234567890", "wma1234567890", "wrk1234567890"]);
    });
  });
  ...
  describe("removeTemplate", () => {
    it("removes the specified id out of a list of templates", () => {
      ...
      expect(templatization.getIdsInTemplatesList(solnTemplates)).toEqual(["map1234567890", "wrk1234567890"]);
    });
    ...
    it("should handle helper services", () => {
      const template: any = {
        item: {
          ...
          geocodeProp: "{{organization.helperServices.geocode:getDefaultLocatorURL}}",
        },
      };
      ...
      const expected: any = {
        item: {
          ...
          routeProp: "https://route.arcgis.com/arcgis/rest/services/World/Route/NAServer/Route_World",
          geomProp: "https://utility.arcgisonline.com/arcgis/rest/services/Geometry/GeometryServer",
          geocodeProp: "https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer",
        },
      };
      const actual: any = templatization.replaceInTemplate(template, templateDictionary);
      expect(actual).toEqual(expected);
    });
  ...
  describe("replaceTemplate", () => {
    it("returns false when there are no templates", () => {
      const solnTemplates: interfaces.IItemTemplate[] = [];
      const newTemplate: interfaces.IItemTemplate = templates.getItemTemplate("Form");
      const actual: boolean = templatization.replaceTemplate(solnTemplates, "map1234567890", newTemplate);
  ...
  describe("templatizeTerm", () => {
    ...
    it("should handle multiple occurrences of term", () => {
      const context: string = "a sentence with multiple aTerms in it: aTerm, aTerm";
      const term: string = "aTerm";
      const suffix: string = ".itemId";
      const expected: string = "a sentence with multiple {{aTerm.itemId}}s in it: {{aTerm.itemId}}, {{aTerm.itemId}}";
  ...
  describe("templatizeFieldReferences", () => {
    it("will not templatize fieldnames that have already been templatized", () => {
      const obj: any = {
        ...
        icon: "{{portalBaseUrl}}${itemId}/resources/inConfig/32951462444715296.png",
        generic: "{{test}}",
      ...
      const expected: any = {
        ...
        generic3: "{{something.test.something}}",
        generic4: "{{something.test.something.somethingElse}}",
        generic5: '$feature["COUNTY_ID.{{0019226378376276.layer0.fields.name.name}}"]',
        generic6: '$feature["{{0019226378376276.layer0.fields.name.name}}"]',
      };

      const actual = templatization.templatizeFieldReferences(obj, fields, basePath);
      expect(actual).toEqual(expected);
    });
    ...
    it("will templatize keys", () => {
      ...
      const expected: any = {
        "{{0019226378376276.layer0.fields.name.name}}": "{{0019226378376276.layer0.fields.name.name}}",
        "{{0019226378376276.layer0.fields.test.name}}": {
          prop: "{{0019226378376276.layer0.fields.name.name}}: ",
        },
      };

      const actual = templatization.templatizeFieldReferences(obj, fields, basePath, true);
      expect(actual).toEqual(expected);
    });
  });
