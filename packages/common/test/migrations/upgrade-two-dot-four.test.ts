...
describe("Upgrade 2.4 ::", () => {
  ...
  it("replaces key tokens with ids", () => {
    ...
    expect(chk.data.metadata.chk1).toBe("behold {{fakeId.itemId}} it changed", "should swap key.item.id => id.itemId");
  ...
  it("reworks hub asset names", () => {
    ...
    expect(tmpl.resources[0]).toBe("fakeId-somefile.png", "should strip the old id out of the filename");
  });
  it("reworks hub asset names, dropping leading folders", () => {
    ...
    expect(tmpl.resources[1]).toBe("fakeId-foo.png", "should strip the old id and folder out of the filename");
  });
  it("adds estimatedDeploymentCostFactor", () => {
    const m = cloneObject(defaultModel);
    m.data.templates[0].estimatedDeploymentCostFactor = 2;
    const chk = _upgradeTwoDotFour(m);
    expect(chk.data.templates[1].estimatedDeploymentCostFactor).toBe(1, "should add cost factor of 1 if missing");
  });
});
