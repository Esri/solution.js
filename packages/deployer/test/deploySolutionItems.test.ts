...
describe("Module `deploySolutionItems`", () => {
  ...
  describe("swizzleFormZipFile", () => {
    ...
    it("swizzles the form zip file", async () => {
      ...
      const swizzledZip = await zipUtils.swizzleFormZipFile(zipAsBlob, sourceItemId, destinationItemId);
    ...
    it("catches the inability to load the zip", async () => {
      const blob = new Blob([""], { type: "application/zip" });
      zipUtils.swizzleFormZipFile(blob, sourceItemId, destinationItemId)
        .then(() => {
          return Promise.reject("Should not have swizzled the zip file");
        })
        .catch(() => {
          return Promise.resolve();
        });
...
async function _compareZips(
  ...
  const zip1Files = await zipUtils._getZipFileContents(zip1);
  const zip2Files = await zipUtils._getZipFileContents(zip2);
...
function _generateFormZip(
  ...
  zip.file("esriinfo/form.json", `{"layerName":"survey","portalUrl":"https://www.arcgis.com","questions":[{"id":"field_1","position":0,"fieldName":"untitled_question_1","name":"untitled_question_1","type":"esriQuestionTypeText","isRequired":false,"fieldType":"esriFieldTypeString","validation":{"valueRange":{"isEnabled":false},"inputMask":{"isEnabled":false,"customFormat":""}},"label":"Untitled question 1","description":null}],"version":"3.19"}`);
