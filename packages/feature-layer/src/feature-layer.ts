...
export function convertItemToTemplate(
  ...
  return new Promise<common.IItemTemplate>((resolve, reject) => {
    // Init template
    const template: common.IItemTemplate = common.createInitializedItemTemplate(itemInfo);

    const hasInvalidDesignations: boolean = common.hasInvalidGroupDesignations(itemInfo.groupDesignations);
    if (hasInvalidDesignations) {
      common.updateTemplateForInvalidDesignations(template, srcAuthentication).then(
      ...
      common.getItemDataAsJson(template.item.id, srcAuthentication).then((data) => {
        ...
        common.getServiceLayersAndTables(template, srcAuthentication).then(
          (itemTemplate) => {
            ...
            itemTemplate.item.modified = Math.max(
...
export function createItemFromTemplate(
  ...
  return new Promise<common.ICreateItemFromTemplateResponse>((resolve) => {
    ...
    const popupInfos: common.IPopupInfos = common.cachePopupInfos(newItemTemplate.data);

    // Create the item, then update its URL with its new id
    common.createFeatureService(newItemTemplate, destinationAuthentication, templateDictionary).then(
      (createResponse) => {
        if (createResponse.success) {
          ...
          if (
            ...
            common.removeItem(createResponse.serviceItemId, destinationAuthentication).then(
              () => resolve(common.generateEmptyCreationResponse(template.type)),
              () => resolve(common.generateEmptyCreationResponse(template.type)),
            );
          } else {
                        ...
                        if (
                          ...
                          common.getItemBase(newItemTemplate.itemId, destinationAuthentication).then(
                            (updatedItem) => {
                              ...
                              if (
                                common.getProp(newItemTemplate, "item.url") &&
...
//TODO: function doc
export function _mostRecentlyEditedLayer(layerOrTableList: any[]): any {
