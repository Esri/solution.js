...
/**
 * The portion of a Web Scene URL between the server and the map id.
 * @protected
 */
const WEBMAP_APP_URL_PART: string = "/home/webscene/viewer.html?webscene=";

// ------------------------------------------------------------------------------------------------------------------ //

/**
 * Converts an AGOL webscene item to a template.
 *
 * @param itemTemplate Template for the webscene item
    ...
    _extractDependencies(itemTemplate, authentication).then(
      (results: any) => {
        ...
        if (itemTemplate.data) {
          _templatizeWebsceneLayerIdsAndUrls(
...
/**
 * Gets the ids of the dependencies of an AGOL webscene item.
 *
 * @param itemTemplate A webscene item whose dependencies are sought
...
export function _extractDependencies(
  ...
  return new Promise<any>((resolve, reject) => {
    ...
    if (itemTemplate.data) {
      ...
      dependencies = _getWebsceneLayerIds(layersAndTables);
 ...
 * @protected
 */
export function _getWebsceneLayerIds(layerList: any[]): string[] {
...
/**
 * Templatizes the url and item id for layers or tables within the webscene.
 *
 * @param layerList List of map layers or tables
 ...
 * @protected
 */
export function _templatizeWebsceneLayerIdsAndUrls(
  layerList = [] as any[],
  urlHash: any
): void {
