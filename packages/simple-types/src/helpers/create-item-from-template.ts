        ...
        .then(
          (createResponse) => {
            ...
            if (
              ...
              common.removeItem(createResponse.id, destinationAuthentication).then(
                () => resolve(common.generateEmptyCreationResponse(template.type)),
                () => resolve(common.generateEmptyCreationResponse(template.type)),
              );
            } else {
              ...
              templateDictionary[template.itemId] = {
                ...
                itemUrl:
                  (templateDictionary["portalBaseUrl"] as string) + "/sharing/rest/content/items/" + createResponse.id,
              };
              ...
              if (newItemTemplate.type === "Web Mapping Application" && template.data) {
                common.setProp(newItemTemplate, "data.appItemId", createResponse.id);
              }
              const postProcess: boolean = common.hasUnresolvedVariables(newItemTemplate.data);
              ...
              if (
                ...
                template.data &&
                common.hasAnyKeyword(template, ["WAB2D", "WAB3D", "Web AppBuilder"])
              ) {
