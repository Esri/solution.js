...
export function getFolders(
  ...
  return common.getUserContent({ authentication })
    .then((response: common.IUserContentResponse) => {
      return response.folders.map(
        (folder: common.IFolder) => {
          return {
            id: folder.id,
            title: folder.title
          } as ISolutionInfoCard;
...
export function getTemplates(
  ...
  const searches: Array<Promise<common.ISearchResult<common.IItem>>> = [];
