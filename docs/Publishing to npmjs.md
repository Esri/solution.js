...
* \[ \] Run `npm run release:review` in a Windows shell
* \[ \] Check and fix CHANGELOG.md
* \[ \] Run `npm run release:publish` in a Windows shell
* \[ \] Check that publishing worked using `check_npm_package_versions.html` in a browser
* \[ \] Run `npm run release:publish-retry` in a Windows shell as needed until all packages are published
